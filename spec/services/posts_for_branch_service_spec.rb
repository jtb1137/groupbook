...
describe PostsForBranchService do
  context "#call" do
    ...
    let(:post) do
      create(:post, title: "a very fun post", category_id: category_id)
    end

    it "returns posts filtered by a branch" do
      not_included_posts
      category
      included_posts = create_list(:post, 2, category_id: category_id)
    ...
    it "returns posts filtered by a category name and a search input" do
      ...
      category
      included_post = [] << post
      expect(PostsForBranchService.new({ name: "arts", search: "fun", branch: "hobby" }).call).to(eq included_post)
    end
